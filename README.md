...
### 4.12 Apache Superset — BI / analytics
...
```mermaid
sequenceDiagram
    participant U as Client (curl / dashboard)
...
5. **Agents (on demand or alarm-triggered)** — the Supervisor delegates to
   ...
   memories.
6. **Visualization** — **Superset** provides ad-hoc BI over PostgreSQL;
   `/spc/report` renders control charts; the **observability API** (`:8001`)
   streams board + container health to the admin dashboard.
...
| Profile | Services |
|---------|----------|
| `core` | EMQX, bridge, Kafka, Flink, IoTDB, Postgres, Qdrant, `llm-server`, `embeddings`, `spc-mcp`, agent-orchestrator, observability |
...
**Web UIs:** EMQX `:18083` · Flink `:8081` · Qdrant
...
- **Credentials:** `POSTGRES_*`, `EMQX_*`, `IOTDB_*`, `SUPERSET_*`.
